...
final class Oven: ObservableObject {
  ...
  private func bakePending() async {
    ...
    let folder = defaults.string(forKey: Preference.destinationFolder).map {
    ...
    while let index = items.firstIndex(where: { $0.status == .waiting }) {
      ...
      let result = await Task.detached(priority: .userInitiated) { () -> Result<URL, Error> in
        do {
          let output = Self.destination(
            for: input, suffix: suffix, folder: folder, replace: replace)
  ...
  nonisolated static func destination(for input: URL, suffix: String, folder: URL?, replace: Bool)
    -> URL
  {
